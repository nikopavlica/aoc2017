...
function getDifOfMinMax(row) {
	let min = Number.MAX_VALUE;
	let max = -min;

	for (let i = 0, l = row.length, n; i < l; ++i) {
		n = row[i];
...
function getEvenlyDivisableSumFromRow(row) {
	const len = row.length;
	return row.reduce((s, n1, ix) => {
		if (s) return s;

		for (let i = ix+1; i < len; ++i) {
			const n2 = row[i];
...
function crazySum(inputStr, rowSum) {
	const inputRows = inputStr.split('\n').map(row => row.split(/\s+/).map(n => parseInt(n, 10)));

	return inputRows.reduce((s, row) => s + rowSum(row), 0);
}

console.log(crazySum(input2, getDifOfMinMax));
// console.log(crazySum(input, getDifOfMinMax));

console.log(crazySum(input, getEvenlyDivisableSumFromRow));
