...
jwfm ptjwrbl hhuv uolz adyweh qpj wxyogp igvnojq jmfw pqs fsnirby`.split('\n');

const isPassphraseValid = (phrase) => {
	const buff = {};
	const words = phrase.split(' ')

	const uniqueWords = words.filter(ph => {
		let key = ph.split('');
		key.sort();
		key = key.join('');
		const res = !buff[key];
...
console.log(input.filter(p => isPassphraseValid(p)).length);
